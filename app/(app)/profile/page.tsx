  ...
  return (
    <section className="animate-fade-in space-y-6">
      ...
      <Card className="@container warm-shadow overflow-hidden">
        <div className="relative h-32 bg-linear-to-r from-primary/20 via-primary/10 to-secondary/10">
          {profile?.openToWork && (
            <div className="absolute left-4 top-4">
              <Badge className="gap-1 rounded-full bg-primary/90 px-3 py-1 text-xs font-semibold text-white">
              ...
              <Form {...form}>
                ...
                >
                  ...
                  <div className="flex items-center gap-3 pt-2">
                    ...
                    {statusText && (
                      <p
                        className={`text-xs ${statusText.includes("saved") ? "text-primary" : "text-muted-foreground"}`}
                      >
                        {statusText}
                      </p>
                    )}
...
function ExperienceForm({
  ...
  return (
    <div className="@container space-y-3 rounded-xl border border-primary/30 bg-primary/5 p-4">
      ...
      <div className="flex gap-2">
        <Button size="sm" className="rounded-full bg-primary text-white hover:bg-primary/90" disabled={!values.title || !values.company || !values.startDate || saving} onClick={async () => { setSaving(true); await onSave(values); setSaving(false); }}>
          {saving ? "Saving..." : "Save"}
        </Button>
...
function EducationForm({
  ...
  return (
    <div className="@container space-y-3 rounded-xl border border-primary/30 bg-primary/5 p-4">
      ...
      <div className="flex gap-2">
        <Button size="sm" className="rounded-full bg-primary text-white hover:bg-primary/90" disabled={!values.school || saving} onClick={async () => { setSaving(true); await onSave(values); setSaving(false); }}>
          {saving ? "Saving..." : "Save"}
        </Button>
        <Button size="sm" variant="ghost" className="rounded-full" onClick={onCancel}>
          <X className="mr-1 size-3.5" /> Cancel
