...
function FeatureCard({
  ...
  const content = (
    <div className="flex items-start gap-3">
      <div
        className={`flex size-8 shrink-0 items-center justify-center rounded-lg ${enabled ? "bg-primary/10 text-primary" : "bg-secondary text-muted-foreground"}`}
      >
      ...
      <div className="min-w-0">
        ...
        <div className="mt-1.5 flex items-center gap-1.5">
          <p
            className={`text-xs font-medium ${enabled ? "text-primary" : "text-amber-accent"}`}
          >
  ...
  if (!enabled) {
    return (
      <Link
        ...
        className="rounded-xl border border-border bg-card p-4 transition-colors hover:border-secondary/40 hover:bg-secondary/5"
      >
        {content}
      </Link>
    );
